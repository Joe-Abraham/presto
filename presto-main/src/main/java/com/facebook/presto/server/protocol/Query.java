...
class Query
{
    ...
    private synchronized void setQueryOutputInfo(QueryExecution.QueryOutputInfo outputInfo)
    {
        ...
        if (columns == null) {
            ...
            Set<ClientCapabilities> clientCapabilities = session.getClientCapabilities();
            ImmutableList.Builder<Column> list = ImmutableList.builder();
            for (int i = 0; i < columnNames.size(); i++) {
                list.add(toColumn(columnNames.get(i), columnTypes.get(i), clientCapabilities));
            }
            columns = list.build();
     ...
     * (e.g. {@code timestamp}) or precision-specific forms (e.g. {@code timestamp_microseconds}).
     */
    private static Column toColumn(String name, Type type, Set<ClientCapabilities> clientCapabilities)
    {
        String typeString = TimestampTypeCompatibility.getTypeStringForClient(type, clientCapabilities);
        return new Column(name, typeString);
    }
