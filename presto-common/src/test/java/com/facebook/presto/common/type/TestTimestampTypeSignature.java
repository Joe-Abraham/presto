...
public class TestTimestampTypeSignature
{
    ...
    {
        InvalidFunctionArgumentException e = expectThrows(InvalidFunctionArgumentException.class,
                () -> TimestampParametricType.TIMESTAMP.createType(ImmutableList.of(TypeParameter.of(-1L))));
        assertTrue(e.getMessage().contains("-1"), "Expected message to include the offending value: " + e.getMessage());
    }
    ...
    {
        InvalidFunctionArgumentException e = expectThrows(InvalidFunctionArgumentException.class,
                () -> TimestampParametricType.TIMESTAMP.createType(ImmutableList.of(TypeParameter.of((long) MAX_PRECISION + 1))));
        assertTrue(e.getMessage().contains(String.valueOf(MAX_PRECISION + 1)), "Expected message to include the offending value: " + e.getMessage());
    }

    @Test
    public void testPrecisionOverflowingIntDoesNotWrapIntoValidRange()
    {
        // (int) (1L << 32 | 5) == 5, which is a valid precision. Without a range check on the
        // long value before narrowing, this would silently resolve to TIMESTAMP(5) instead of failing.
        long overflowing = (1L << 32) + 5;
        InvalidFunctionArgumentException e = expectThrows(InvalidFunctionArgumentException.class,
                () -> TimestampParametricType.TIMESTAMP.createType(ImmutableList.of(TypeParameter.of(overflowing))));
        assertTrue(e.getMessage().contains(String.valueOf(overflowing)), "Expected message to include the offending value: " + e.getMessage());
    }

    @Test
    public void testWrongParameterCountThrows()
    {
        InvalidFunctionArgumentException e = expectThrows(InvalidFunctionArgumentException.class,
                () -> TimestampParametricType.TIMESTAMP.createType(ImmutableList.of(TypeParameter.of(3L), TypeParameter.of(6L))));
        assertTrue(e.getMessage().contains("2"), "Expected message to include the offending parameter count: " + e.getMessage());
    }

    @Test
    public void testNonLiteralParameterThrows()
    {
        expectThrows(InvalidFunctionArgumentException.class,
                () -> TimestampParametricType.TIMESTAMP.createType(ImmutableList.of(TypeParameter.of(VarcharType.VARCHAR))));
    }
}
