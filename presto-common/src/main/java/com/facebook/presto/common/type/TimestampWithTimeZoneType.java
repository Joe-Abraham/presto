...
/**
 * TIMESTAMP WITH TIME ZONE type with precision 0..{@value LongTimestampWithTimeZoneType#MAX_PRECISION_TSTZ}.
 * <p>
 ...
 * <p>
 * Precision 4-{@value LongTimestampWithTimeZoneType#MAX_PRECISION_TSTZ} (long timestamps with time zone)
 * are stored as 12 bytes using {@link com.facebook.presto.common.block.Fixed12Block}: the first 8 bytes
 * store epoch milliseconds and the last 4 bytes store {@code (nanosOfMilli << 12) | (timeZoneKey & 0xFFF)}.
 * Precision 10-12 is not supported because the packed 32-bit word has no remaining bits for
 * sub-nanosecond values — see {@link LongTimestampWithTimeZoneType#MAX_PRECISION_TSTZ}.
 * <p>
        ...
        implements FixedWidthType
{
    ...
    {
        super(javaType);
        if (precision < 0 || precision > LongTimestampWithTimeZoneType.MAX_PRECISION_TSTZ) {
            throw new IllegalArgumentException(format(
                    "TIMESTAMP WITH TIME ZONE precision must be in range [0, %d]: %s",
                    LongTimestampWithTimeZoneType.MAX_PRECISION_TSTZ, precision));
        }
        this.precision = precision;
    }

    /**
     * Creates a TimestampWithTimeZoneType with the given precision (0..{@value LongTimestampWithTimeZoneType#MAX_PRECISION_TSTZ}).
     * Precision 10-12 is not supported because the {@link LongTimestampWithTimeZoneType} storage format
     * cannot represent sub-nanosecond values alongside a timezone key.
     *
     * @throws IllegalArgumentException if {@code precision} is outside [0, {@value LongTimestampWithTimeZoneType#MAX_PRECISION_TSTZ}]
     */
