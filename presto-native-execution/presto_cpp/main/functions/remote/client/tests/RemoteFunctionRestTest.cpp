...
class RemoteFunctionRestTest
 ...
 private:
  ...
  void registerRemoteFunctionHelper(
    ...
    PrestoRemoteFunctionsMetadata metadata;
    metadata.serdeFormat = GetParam();
    metadata.location = baseLocation + "/" + functionName;
    registerPrestoRemoteFunction(functionName, {finalSignature}, metadata);
  }
  ...
  void registerRemoteFunctions() const {
    ...
    registerPrestoRemoteFunction("remote_round", roundSignatures, metadata);
  }
  ...
  static constexpr auto kHostAddress = "http://127.0.0.1:{}";
