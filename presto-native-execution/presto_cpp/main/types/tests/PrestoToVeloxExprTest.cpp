...
TEST_F(RemoteFunctionTest, HandlesRestFunctionCorrectly) {
  try {
    ...
    auto arg0 = std::dynamic_pointer_cast<const core::ConstantTypedExpr>(callExpr->inputs()[0]);
    auto arg1 = std::dynamic_pointer_cast<const core::ConstantTypedExpr>(callExpr->inputs()[1]);
...
TEST_F(RemoteFunctionTest, UnsupportedSerdeFormat) {
  ...
  VELOX_ASSERT_THROW( converter_->toVeloxExpr(testExpr), "presto_page serde is expected by remote function server but got : 'spark_unsafe_rows'" );
}
